...
import { Plus, User, Coins, Timer, ArrowRightLeft, Search, Filter, Gem, ChevronDown, X, ArrowUpDown } from 'lucide-react';
...
export function TradePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-200 to-blue-300 pt-40 pb-24">
      {/* Header */}
      <div className="fixed top-0 left-0 right-0 bg-gradient-to-b from-blue-200 to-blue-200/95 pt-4 pb-4 px-4 z-30">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-white drop-shadow-lg">Trade Center</h1>
            <p className="text-white/80 text-sm mt-1">Exchange your characters!</p>
          </div>
        </div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-bold text-gray-700">Trade Offer</h3>
                  </div>
                  <div className="bg-blue-100 rounded-full px-3 py-1 flex items-center gap-1.5">
                    <ArrowRightLeft className="w-4 h-4 text-blue-600" />
                    <span className="text-xs font-semibold text-blue-600">Exchange</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-3">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-16 h-16 rounded-xl overflow-hidden ring-2 bg-gradient-to-br ${getRarityStyle(offer.have.rarity)}`}>
                        <img
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-gray-800">{offer.have.name}</span>
                          <div className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                            offer.have.rarity === 'legendary' ? 'bg-yellow-100 text-yellow-700' :
                            offer.have.rarity === 'epic' ? 'bg-purple-100 text-purple-700' :
                            offer.have.rarity === 'rare' ? 'bg-blue-100 text-blue-700' :
                            'bg-gray-100 text-gray-700'
                          }`}>
                            {offer.have.rarity.charAt(0).toUpperCase() + offer.have.rarity.slice(1)}
                          </div>
                        </div>
                        <span className={`text-sm font-medium ${getRarityStyle(offer.have.rarity).split(' ')[2]} block mt-1`}>
                          {offer.have.discount}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-3">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-16 h-16 rounded-xl overflow-hidden ring-2 bg-gradient-to-br ${getRarityStyle(offer.want.rarity)}`}>
                        <img
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-gray-800">{offer.want.name}</span>
                          <div className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                            offer.want.rarity === 'legendary' ? 'bg-yellow-100 text-yellow-700' :
                            offer.want.rarity === 'epic' ? 'bg-purple-100 text-purple-700' :
                            offer.want.rarity === 'rare' ? 'bg-blue-100 text-blue-700' :
                            'bg-gray-100 text-gray-700'
                          }`}>
                            {offer.want.rarity.charAt(0).toUpperCase() + offer.want.rarity.slice(1)}
                          </div>
                        </div>
                        <span className={`text-sm font-medium ${getRarityStyle(offer.want.rarity).split(' ')[2]} block mt-1`}>
                          {offer.want.discount}
                        </span>
                      </div>
                    </div>
