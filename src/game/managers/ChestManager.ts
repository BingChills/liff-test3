...
export class ChestManager {
   ...
   private handleCollision(
      ...
      if (this.player.isPlayerMoving()) {
         ...
         const distance = Phaser.Math.Distance.Between(playerSprite.x, playerSprite.y, clickedChest.x, clickedChest.y)
         if (distance < 50) {
   ...
   public getNearestChest(): Physics.Arcade.Sprite | null {
      ...
      for (let i = 0; i < chestSprites.length; i++) {
         ...
         const distance = Phaser.Math.Distance.Between(playerSprite.x, playerSprite.y, chestSprite.x, chestSprite.y)
   ...
   private collectChest(chestSprite: Physics.Arcade.Sprite): void {
      ...
      const pointAmount = Phaser.Math.Between(rarity.pointRange[0], rarity.pointRange[1])
   ...
   private spawnPointOrb(x: number, y: number, amount: number): void {
      ...
      const speed = 100
      pointOrb.setVelocity(Math.cos(angle) * speed, Math.sin(angle) * speed)
