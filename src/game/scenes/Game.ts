...
export class Game extends Scene {
   ...
   preload() {
      try {
         ...
         this.load.spritesheet('ninjaTurtle_walk', 'characters/ninja-turtle/walk.png', {
            frameWidth: 64,
            frameHeight: 64
         })
         this.load.spritesheet('ninjaTurtle_idle', 'characters/ninja-turtle/idle.png', {
            frameWidth: 64,
            frameHeight: 64
         })
         ...
         this.load.spritesheet('hedgehog_walk', 'characters/hedgehog/walk-south.png', {
            frameWidth: 64,
            frameHeight: 64
         })
         this.load.spritesheet('hedgehog_idle', 'characters/hedgehog/idle.png', {
            frameWidth: 64,
            frameHeight: 64
         })
   ...
   create() {
      ...
      this.chestManager = new ChestManager(this, this.player, gameConfig, this.coupons, (newScore: number) => {
         this.score = newScore
      ...
      this.autoModeController = new AutoModeController(this, this.player, this.chestManager)
   ...
   private setupEventListeners(): void {
      ...
      this.gameEventBus.on('chestOpened', (data: { coupon: GameCoupon; points: number }) => {
         ...
         // Emit points collected event to UI if points were awarded
         if (data.points > 0) {
            EventBus.emit('pointsCollected', data.points)
         }
      })
   }
   ...
   private setupPlayerInput(): void {
      this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
         ...
         const distanceToButton = Phaser.Math.Distance.Between(pointer.x, pointer.y, autoButtonX, autoButtonY)
