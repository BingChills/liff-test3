            ...
            >
               ...
               {hasActiveFilters && (
                  <div className='w-5 h-5 rounded-full bg-white text-blue-500 text-xs font-bold flex items-center justify-center'>
                     {[statusFilter !== 'active', daysFilter !== 'all'].filter(Boolean).length}
                  </div>
               )}
            </button>
         </div>
         ...
         <div className='px-4 space-y-4 mt-4'>
            {filteredCoupons.length === 0 ? (
               <div className='bg-white/95 backdrop-blur-sm rounded-2xl p-8 text-center'>
                  ...
                  <p className='text-gray-600'>Try adjusting your filters to see more coupons.</p>
                  {(hasActiveFilters || searchQuery) && (
                     <button
                        onClick={clearFilters}
               ...
               filteredCoupons.map((coupon) => (
                  <div
                     ...
                     className={`bg-white rounded-2xl shadow-lg overflow-hidden transform transition-all duration-200 ${
                        coupon.isUsed || coupon.expiresIn === 0 ? 'opacity-60' : 'hover:scale-[1.02]'
                     }`}
                  >
                        ...
                        <div className='flex-1'>
                           ...
                           <div
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full mt-2 ${
                                 coupon.isUsed ? 'bg-gray-100 text-gray-600' : getExpiryColor(coupon.expiresIn)
                              }`}
                           >
                              {getExpiryIcon(coupon.expiresIn)}
                              <span className='text-sm font-medium'>{getExpiryText(coupon.expiresIn)}</span>
                           </div>
                        </div>
            ...
            >
               ...
               >
                  ...
                  >
                     ...
                     <span className='text-sm font-medium'>{getExpiryText(selectedCoupon.expiresIn)}</span>
                  </div>
