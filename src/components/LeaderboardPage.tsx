...
const PRIZES: Record<number, PrizeInfo> = {
   1: {
      ...
      icon: '',
      rewards: ['Exclusive Legendary Character', '10,000 Gems', 'Special Profile Frame', "Champion's Trophy"]
   },
   2: {
      ...
      icon: '',
      rewards: ['Epic Character of Choice', '5,000 Gems', "Master's Badge", 'Rare Profile Banner']
   },
   3: {
      ...
      icon: '',
      rewards: ['Rare Character of Choice', '2,500 Gems', 'Elite Title', 'Special Emotes Pack']
   }
}
...
const generateLeaderboardData = (): LeaderboardEntry[] => {
   ...
   const additionalEntries = Array.from({ length: 97 }, (_, i) => ({
      ...
      points: Number((5 - i * 0.05).toFixed(1)),
      characters: [
         'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?auto=format&fit=crop&w=50&q=80',
         'https://images.unsplash.com/photo-1573865526739-10659fec78a5?auto=format&fit=crop&w=50&q=80',
         'https://images.unsplash.com/photo-1548802673-380ab8ebc7b7?auto=format&fit=crop&w=50&q=80'
            ...
            {leaderboardData.map((entry) => {
               const getBgColor = (rank: number, isCurrentUser?: boolean) => {
                  ...
                  switch (rank) {
                     ...
                     default:
                        return 'bg-gradient-to-r from-gray-500/90 to-gray-600/90'
                  }
               }
               ...
               const getRankIcon = (rank: number) => {
                  switch (rank) {
                     case 1:
                        return <Crown className='w-5 h-5 text-yellow-300 drop-shadow-glow animate-pulse' />
                     case 2:
                        return <Star className='w-5 h-5 text-gray-200 drop-shadow-glow animate-pulse' />
                     case 3:
                        return <Medal className='w-5 h-5 text-amber-500 drop-shadow-glow animate-pulse' />
                     default:
                        return null
                  }
               }
                ...
                transition-all duration-200 backdrop-blur-sm relative`}
                  >
                     ...
                     <div className='w-14 h-14 rounded-xl overflow-hidden ring-2 ring-white/30'>
                        <img src={entry.avatar} alt={`Rank ${entry.rank}`} className='w-full h-full object-cover' />
                     </div>

                     <div className='flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-full'>
                        <Coins className='w-4 h-4 text-yellow-400' />
                        <span className='text-sm font-bold text-white'>{entry.points}m</span>
                     </div>
            ...
            >
               ...
               >
                  <div className='flex items-center justify-center gap-3 mb-8'>
                     ...
                     <div>
                        <h3 className='text-3xl font-black text-white tracking-tight'>{PRIZES[selectedRank].title}</h3>
         ...
         {currentUserEntry && (
            <div className='fixed bottom-20 left-0 right-0 flex justify-center px-4 z-30'>
               <div className='bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-4 shadow-xl flex items-center gap-4 ring-2 ring-white/30 backdrop-blur-sm max-w-[95%]'>
                  ...
                  <div className='w-14 h-14 rounded-xl overflow-hidden ring-2 ring-white/30'>
                     <img src={currentUserEntry.avatar} alt='Your Rank' className='w-full h-full object-cover' />
                  </div>
                  <div className='flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-full'>
                     <Coins className='w-4 h-4 text-yellow-400' />
                     <span className='text-sm font-bold text-white'>{currentUserEntry.points}m</span>
                  </div>
                  <div className='flex justify-end gap-2 overflow-x-auto max-w-[40%]'>
                     {currentUserEntry.characters.slice(0, 3).map((character, index) => (
                        <div
                           ...
                           className='w-10 h-10 rounded-full overflow-hidden ring-2 ring-white/30 flex-shrink-0'
                        >
                           <img src={character} alt={`Character ${index + 1}`} className='w-full h-full object-cover' />
                        </div>
                     ))}
                  </div>
               </div>
