    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-blue-200 via-blue-300 to-blue-400 flex flex-col">
            ...
            >
                {leaderboardData.map((entry) => {
                    ...
                    const getRankIcon = (rank: number) => {
                        switch (rank) {
                            case 1:
                                return (
                                    <Crown className="w-5 h-5 text-yellow-300 drop-shadow-glow animate-pulse" />
                                );
                            case 2:
                                return (
                                    <Star className="w-5 h-5 text-gray-200 drop-shadow-glow animate-pulse" />
                                );
                            case 3:
                                return (
                                    <Medal className="w-5 h-5 text-amber-500 drop-shadow-glow animate-pulse" />
                                );
                            default:
                                return null;
                        }
                    ...
                    const isNearCurrentUser =
                        Math.abs(entry.rank - (currentUserEntry?.rank || 0)) <=
                        2;
                ...
                ${
                    entry.rank <= 3
                        ? "cursor-pointer transform hover:scale-[1.02] active:scale-95"
                        : ""
                }
                ...
                >
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            {PRIZES[selectedRank].rewards.map(
                                (reward, index) => (
                                    <div
                                        key={index}
                ...
                <div className="fixed bottom-20 left-0 right-0 flex justify-center px-4 z-30">
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-4 shadow-xl flex items-center gap-4 ring-2 ring-white/30 backdrop-blur-sm max-w-[95%]">
                        ...
                        <div className="flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-full">
                            <Coins className="w-4 h-4 text-yellow-400" />
                            <span className="text-sm font-bold text-white">
                                {currentUserEntry.coins}m
                            </span>
                        </div>
                        <div className="flex justify-end gap-2 overflow-x-auto max-w-[40%]">
                            {currentUserEntry.characters
                                .slice(0, 3)
                                .map((character, index) => (
                                    <div
                                        key={index}
