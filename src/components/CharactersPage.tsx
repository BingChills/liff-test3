...
import { User, Filter, Search, Cat, Gem, ChevronDown, Timer, Coins } from 'lucide-react';
import { useGameState, Character } from '../state/gameState';
import PageHeader from './PageHeader';

const CharactersPage = () => {
  const { characters, stores, selectedStore, setSelectedStore, setCharacters, stamina, score } = useGameState();
  const [selectedCompany, setSelectedCompany] = useState<string | null>(null);
  const [showStoreSelector, setShowStoreSelector] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Filter characters based on company
  const filteredCharacters = selectedCompany
    ? characters.filter(char => char.company === selectedCompany)
    : characters;
      ...
      <div className="px-3 mt-4 mb-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {usingCharacters.map(char => (
            ...
            >
              <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full ${getStoreColor(stores.find(s => s.name === char.company)?.color || 'emerald')} flex items-center justify-center`}>
