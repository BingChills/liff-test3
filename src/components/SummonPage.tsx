...
const CHARACTERS: Character[] = [
  // These would be actual characters in a real app
  {
    id: 'char-1',
    name: 'Starbucks Character',
    image: 'https://placehold.co/400x400/green/white?text=SB',
    rarity: 'legendary',
    discount: '30% off at Starbucks',
    isUsing: false,
    company: 'Starbucks'
  },
  {
    id: 'char-2',
    name: 'McDonalds Character',
    image: 'https://placehold.co/400x400/red/yellow?text=MD',
    rarity: 'epic',
    discount: '25% off at McDonalds',
    isUsing: false,
    company: 'McDonalds'
  },
  {
    id: 'char-3',
    name: 'KFC Character',
    image: 'https://placehold.co/400x400/red/white?text=KFC',
    rarity: 'rare',
    discount: '20% off at KFC',
    isUsing: false,
    company: 'KFC'
  },
  {
    id: 'char-4',
    name: 'Subway Character',
    image: 'https://placehold.co/400x400/green/yellow?text=SW',
    rarity: 'common',
    discount: '15% off at Subway',
    isUsing: false,
    company: 'Subway'
  }
];
...
const SummonPage = () => {
  ...
  const handleSummon = (isTenDraw: boolean) => {
    ...
    if (!selectedStore || selectedStore.point < drawCost) {
      console.log(`You only have ${selectedStore?.point || 0} ${selectedStore?.name || ''} points. Need ${drawCost} points.`);
      return;
    }
      ...
      <div className="mt-4 px-4">
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full ${getStoreColor(selectedStore?.color || 'blue')} flex items-center justify-center`}>
              <Gem className="w-5 h-5 text-white" />
            </div>
            <div>
              <h3 className="font-bold">{selectedStore?.name || 'Select Store'}</h3>
              <p className="text-sm text-gray-500">{selectedStore ? `${selectedStore.point} points available` : 'Choose a store to summon from'}</p>
            </div>
          </div>
          <ChevronDown className={`text-gray-500 transition-transform ${showStoreSelector ? 'rotate-180' : ''}`} />
        </div>
        ...
        {showStoreSelector && (
          <div className="mt-2 bg-white/90 rounded-xl overflow-hidden shadow-lg">
            {stores.map(store => (
              ...
              >
                <div className={`w-10 h-10 rounded-full ${getStoreColor(store.color)} flex items-center justify-center`}>
                  <Gem className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className="font-bold">{store.name}</h3>
                  <p className="text-sm text-gray-500">{store.point} points available</p>
                </div>
              </div>
            ))}
          </div>
  ...
  useEffect(() => {
    if (isVisible && !showCharacters) {
      ...
      const newCharacters = Array.from({ length: drawCount }, () => {
        ...
        return possibleCharacters.length > 0 
          ? possibleCharacters[Math.floor(Math.random() * possibleCharacters.length)]
          : CHARACTERS[0]; // Fallback to first character if none match the rarity
      });
        ...
        <div className="w-full max-w-4xl p-4 overflow-y-auto max-h-[90vh]">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {characters.map((char, index) => (
              <div key={index} className="transform animate-scale-up" style={{ animationDelay: `${index * 0.1}s` }}>
                <div className={`rounded-2xl overflow-hidden bg-gradient-to-br ${
                  char.rarity === 'legendary' ? 'from-yellow-400 to-yellow-500' :
                  char.rarity === 'epic' ? 'from-purple-400 to-purple-500' :
                  char.rarity === 'rare' ? 'from-blue-400 to-blue-500' :
                  'from-gray-400 to-gray-500'
                } p-3 shadow-2xl ${
                  char.rarity === 'legendary' ? 'animate-legendary-shine' :
                  char.rarity === 'epic' ? 'animate-epic-shine' :
                  char.rarity === 'rare' ? 'animate-rare-shine' : ''
                }`}>
                  <div className="bg-white/90 rounded-xl p-3 space-y-3">
                    ...
                    <div className="text-center">
                      ...
                      <div className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold mb-1 ${
                        char.rarity === 'legendary' ? 'bg-yellow-100 text-yellow-700' :
                        char.rarity === 'epic' ? 'bg-purple-100 text-purple-700' :
                        char.rarity === 'rare' ? 'bg-blue-100 text-blue-700' :
